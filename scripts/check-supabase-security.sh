...
# husky pre-push hook, see .husky/pre-push) and in CI (see
# .github/workflows/ci.yml's supabase-security-gate job). Uses
# `supabase db advisors` against the LINKED REMOTE project (the same
# engine that powers the Advisors tab in the dashboard and the MCP
# `get_advisors` tool) so the check reflects the database's actual live
# state, not just the migration files in this branch.
#
# Exits non-zero — and blocks the push — on any security advisory at
# warn level or above that isn't listed in
# scripts/supabase-security-allowlist.json.
set -euo pipefail
...
if [ "$STATUS" -ne 0 ]; then
  ...
  echo "   Fix them (see the supabase-postgres-best-practices / supabase skill security checklist)," >&2
