...
const QUOTATION_PDF_SELECT =
  "id, organization_id, updated_at, quotation_number, issue_date, valid_until, currency, subtotal, discount_total, tax_total, grand_total, scope_of_work, deliverables, timeline, assumptions, exclusions, terms, quotation_items(*), clients(name, email), organizations(name, email, phone, gstin, address_line_1, address_line_2, city, state, pincode, bank_account_name, bank_account_number, bank_ifsc, bank_name, upi_id)";

export async function loadQuotationPdfData(
  supabase: SupabaseClient<Database>,
  quotationId: string,
): Promise<{ organizationId: string; updatedAt: string; data: QuotationPdfData } | null> {
